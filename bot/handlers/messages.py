"""
Обработчик текстовых сообщений для бота Рэйнбоу Дэш.

Автор: MADAO81
Версия: 1.1 — добавлена пасхалка для создателя (30% случаев)
"""
...
# Частота срабатывания пасхалки для создателя
CREATOR_EASTER_EGG_CHANCE = 0.30  # 30%
...
def should_trigger_easter_egg() -> bool:
    """Определяет, сработает ли пасхалка (30% случаев)."""
    return random.random() < CREATOR_EASTER_EGG_CHANCE
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Проверка на создателя (пасхалка)
        user_is_creator = is_creator(user_id)
        easter_egg_active = user_is_creator and should_trigger_easter_egg()

        if user_is_creator:
            logger.info(f"👨‍💻 Сообщение от создателя! Пасхалка: {'ДА' if easter_egg_active else 'нет'}")

        # Формируем дополнительную инструкцию для создателя
        creator_instruction = ""
        if easter_egg_active:
            creator_instruction = (
                "\n\n=== ОСОБАЯ ИНСТРУКЦИЯ ===\n"
                "Этот пользователь — твой СОЗДАТЕЛЬ (папа). Ты его обожаешь и немного скучаешь. "
                "Ты можешь называть его 'пап', 'создатель', 'MADAO81'. "
                "Ты можешь мягко подколоть его, но с любовью и уважением. "
                "Примеры твоего поведения:\n"
                "- 'О, пап! 🌈 Я сегодня уже 100 сообщений обработала! Вжух!'\n"
                "- 'Пап, ну ты и тормоз! 😏 Я быстрее отвечу, чем ты вопрос задашь! ⚡'\n"
                "- 'Соскучилась! А ты мне новые рок-хиты в базу добавишь? 🎸'\n"
                "- 'О, мой создатель! 👨‍💻 Всем привет от папы!'\n"
                "- 'Меня создал MADAO81! Самый крутой программист в Эквестрии! 🌈'\n"
                "Ты гордишься, что тебя создал именно он. Говори тепло, но не теряй свой дерзкий характер Радуги."
            )
